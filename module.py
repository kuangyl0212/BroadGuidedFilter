...
class BLS(nn.Module):
    def __init__(self, n_input, n_mapping, n_enhance, n_output):
        super(BLS, self).__init__()
        # define the structure of BLS
        self.feature_mapping = nn.Linear(n_input, n_mapping)
        self.feature_enhancement = nn.Linear(n_mapping, n_enhance)
        self.predict = nn.Linear(n_mapping + n_enhance, n_output)
...
class BroadGuidedFilter(nn.Module):
    def __init__(self, n_input, n_mapping, n_enhance, n_output, radius=1, eps=1e-8,):
        super(BroadGuidedFilter, self).__init__()
        self.bl = BLS(n_input, n_mapping, n_enhance, n_output)
        self.gf = FastGuidedFilter(radius, eps)
